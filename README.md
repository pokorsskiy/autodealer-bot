...
Основной проект — бот-витрина в `showcase_bot/`: каталог решений, заявки на разработку и связь с владельцем.

Запуск: `venv\Scripts\python.exe -m showcase_bot.bot`.
Настройки и ссылки на примеры указаны в `showcase_bot/.env.example`.

Репозиторий содержит основной бот-витрину, а также четыре самостоятельных демонстрационных сценария для автодилера.
